...
def market_sentiment(request):
    ...
    def safe_float(val, default=0.0):
        """Convert to float and replace NaN/Inf with a safe default."""
        try:
            f = float(val)
    ...
    try:
        ...
        for p_symbol in tickers:
            try:
                ...
                # Drop rows where Close is entirely NaN (can happen over weekends)
                ticker_df = ticker_df.dropna(subset=['Close'])
                if ticker_df.empty:
                    continue

                current_price = safe_float(ticker_df['Close'].iloc[-1])

                # Skip if price is still 0 (truly no data)
                if current_price == 0:
                    continue

                if len(ticker_df) >= 2:
                    prev_close = safe_float(ticker_df['Close'].iloc[-2])
                    if prev_close == 0:
                        prev_close = current_price  # avoid div/0
                    change_pct = ((current_price - prev_close) / prev_close) * 100
                    change_val = current_price - prev_close
                else:
                    open_price = safe_float(ticker_df['Open'].iloc[-1])
                    if open_price == 0:
                ...
                if not mc or safe_float(mc) == 0:
                    mc = 1_000_000_000  # Default 1B

                data.append({
                    "symbol": p_symbol,
                    "marketCap": safe_float(mc),
                    "price": round(current_price, 2),
                    "changePercent": round(safe_float(change_pct), 2),
                    "change": round(safe_float(change_val), 2),
                })
